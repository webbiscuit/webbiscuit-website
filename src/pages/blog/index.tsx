...
const BlogLatestPreview = ({title, date, description}: {title?: string, date?: string, description?: string}) => {
  return (
    <Box as="article" borderStyle={'solid'} borderColor="yellow.900" borderWidth={4} padding={2} mb={4}>
...
const BlogPreview = ({title, date, description, image, image_alt, slug}: 
    {title?: string, date?: string, description?: string, image?: IGatsbyImageData, image_alt?: string, slug?: string}) => {
    return (
      <Box as="article" borderStyle={'solid'} borderColor="yellow.900" borderWidth={4} borderBottom={0} padding={2}
        ...
        <HStack spacing='24px'>
          ...
          <Link to={`/blog${slug}`}><Heading size="xl" fontWeight={600}>{title}</Heading></Link>
        ...
        <Box as="section">
          ...
          <Box boxShadow='2xl' p='6' rounded='md' bg='yellow.50'>
          {
            data.allMdx.nodes.map((node, ix, arr) => {
              const image = node.frontmatter?.hero_image ? getImage(node.frontmatter?.hero_image as ImageDataLike) : undefined;

              if (ix === 110) {
                return <BlogLatestPreview 
                  key={node.id}
